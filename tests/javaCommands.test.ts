...
describe("Java command marker invocations", () => {
  ...
  it("wires desktop discovery and accessible custom parameter controls into the canonical renderer", () => {
    ...
    expect(inspector).toContain("function InlinePicker");
    expect(inspector).toContain("role: 'combobox'");
    expect(inspector).toContain("role: 'listbox'");
    expect(inspector).toContain("role: 'option'");
    expect(inspector).toContain("items.length > searchThreshold");
    expect(inspector).toContain("const MAX_RENDERED_PICKER_ITEMS = 80");
    expect(inspector).toContain("filteredItems.slice(0, MAX_RENDERED_PICKER_ITEMS)");
    expect(inspector).toContain("Keep typing to narrow results");
    expect(inspector).toContain("!event.currentTarget.contains(event.relatedTarget)");
