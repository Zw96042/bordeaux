...
For a multi-path autonomous routine, construct `BordeauxRoutineRunner` with the same loaded document, command registry, and condition registry. `start()` returns the first stable path ID. After that trajectory finishes, pass its ID to `completePath(...)`; the runner evaluates sensor decisions, schedules bound between-path commands, and returns the next path ID. An empty result means the routine is complete. The strict reader rejects simulation-only function steps, unknown path references, duplicate node IDs, and oversized trees.
