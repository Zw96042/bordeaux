...
Beta clients may graduate to a newer stable release. Downgrades are never automatic. Bordeaux checks quietly shortly after launch. **Settings → Software updates** and **Check for Updates…** open the shared update panel with release notes, download progress, cancellation, and retry. Closing the panel during a download leaves it running; canceling stops the transfer before another download can begin. After downloading and verifying an update, Bordeaux installs only when the user chooses **Restart and install**. Unsaved edits do not block installation, and closing the app no longer opens a save/discard prompt; save the project explicitly when those edits need to be kept. The install panel remains open while the native updater verifies and starts installation. If restart has not completed after one minute, the panel offers Close and manual installation from All releases; saving remains available, and another native install request is not started while the first is pending. Windows automatic updates use silent installation to retain the existing installation without reopening the setup wizard. The NSIS include also recognizes the updater’s `--updated` handoff from older clients, while manual first-time installs keep the setup wizard. Failed installation restores error recovery without shutting down background services. Services stop through the normal quit cleanup only after installation actually begins quitting the app. After asynchronous cleanup, quit resumes on the next event-loop turn so Electron has finished canceling the initial quit event. `node scripts/verify-update-quit.mjs` exercises production window closure and cleanup using a controlled installer handoff; it does not verify native installer execution.

An older beta already stuck during restart may require a one-time manual installer replacement. The new release cannot repair the older process before installation.
...
The macOS workflows sign DMGs explicitly, then submit and staple each image with `scripts/notarize-release-dmgs.mjs`. Stapling changes the image bytes, so the script regenerates DMG blockmaps and manifest hashes before verification. ZIPs retain the already notarized app and their original hashes.

By default, both GitHub release workflows fail before packaging when either platform's signing configuration is absent. The beta workflow has an explicit `unsigned_windows` dispatch option for temporary unsigned Windows installers. This option bypasses only Windows signing and Authenticode checks, retains package and update-manifest verification, and labels the release notes. Production signing requirements are unchanged. After packaging, macOS jobs verify the Developer ID team, Gatekeeper assessment, and stapled notarization ticket on every DMG and the app inside every ZIP. Windows jobs verify that every setup and portable executable has a valid, timestamped Authenticode signature from `WIN_SIGNER_SHA1`. The workflows publish those checks, artifact SHA-256 hashes, and the verified updater manifest in `signature-evidence-*.json`; signed beta runs and production releases require signed Windows artifacts. Never publish replacement artifacts under an existing release version.
